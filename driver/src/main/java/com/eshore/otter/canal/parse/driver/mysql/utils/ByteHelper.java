...
public abstract class ByteHelper {
    ...
    public static byte[] readNullTerminatedBytes(byte[] data, int index) {
        ...
        for (int i = index; i < data.length; i++) {
            byte item = data[i];
            if (item == com.eshore.otter.canal.parse.driver.mysql.utils.MSC.NULL_TERMINATED_STRING_DELIMITER) {
                break;
            }
            out.write(item);
        }
    ...
    public static void writeNullTerminatedString(String str, ByteArrayOutputStream out) throws IOException {
        ...
        out.write(com.eshore.otter.canal.parse.driver.mysql.utils.MSC.NULL_TERMINATED_STRING_DELIMITER);
    }
