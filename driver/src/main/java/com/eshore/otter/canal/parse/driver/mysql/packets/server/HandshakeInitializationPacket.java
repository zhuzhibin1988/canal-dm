...
import com.eshore.otter.canal.parse.driver.mysql.packets.Capability;
...
public class HandshakeInitializationPacket extends PacketWithHeaderPacket {
    ...
    public void fromBytes(byte[] data) {
        ...
        byte[] serverVersionBytes = com.eshore.otter.canal.parse.driver.mysql.utils.ByteHelper.readNullTerminatedBytes(data, index);
        serverVersion = new String(serverVersionBytes);
        index += (serverVersionBytes.length + 1);
        // 3. read thread_id
        threadId = com.eshore.otter.canal.parse.driver.mysql.utils.ByteHelper.readUnsignedIntLittleEndian(data, index);
        index += 4;
        // 4. read scramble_buff
        seed = com.eshore.otter.canal.parse.driver.mysql.utils.ByteHelper.readFixedLengthBytes(data, index, 8);
        index += 8;
        index += 1; // 1 byte (filler) always 0x00
        // 5. read server_capabilities
        this.serverCapabilities = com.eshore.otter.canal.parse.driver.mysql.utils.ByteHelper.readUnsignedShortLittleEndian(data, index);
        index += 2;
        if (data.length > index) {
            ...
            this.serverStatus = com.eshore.otter.canal.parse.driver.mysql.utils.ByteHelper.readUnsignedShortLittleEndian(data, index);
            index += 2;
            // 8. bypass filtered bytes
            int capabilityFlags2 = com.eshore.otter.canal.parse.driver.mysql.utils.ByteHelper.readUnsignedShortLittleEndian(data, index);
            index += 2;
            ...
            if ((capabilities & com.eshore.otter.canal.parse.driver.mysql.packets.Capability.CLIENT_SECURE_CONNECTION) != 0) {
                ...
                this.restOfScrambleBuff = com.eshore.otter.canal.parse.driver.mysql.utils.ByteHelper.readFixedLengthBytes(data, index, 12);
            }
