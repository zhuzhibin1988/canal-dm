 ...
 */
public class LogMinerSqls {

    private static final Logger LOGGER = LoggerFactory.getLogger(LogMinerSqls.class);
    // ****** RAC specifics *****//
...
//    static final String BUILD_DICTIONARY = "BEGIN DBMS_LOGMNR_D.BUILD (options => DBMS_LOGMNR_D.STORE_IN_REDO_LOGS); END;";
     ...
     */
    public static final String FILES_FOR_MINING = "SELECT * FROM V$LOGMNR_LOGS";
    ...
    public static String allOnlineLogsQuery() {
        return String.format("SELECT MIN(F.MEMBER) AS FILE_NAME, L.NEXT_CHANGE# AS NEXT_CHANGE, F.GROUP#, L.FIRST_CHANGE# AS FIRST_CHANGE, L.STATUS " + " FROM %s L, %s F " + " WHERE F.GROUP# = L.GROUP# AND L.NEXT_CHANGE# > 0 " + " GROUP BY F.GROUP#, L.NEXT_CHANGE#, L.FIRST_CHANGE#, L.STATUS ORDER BY 3", LOG_VIEW, LOGFILE_VIEW);
    }
    ...
    public static String logMiningHistoryDdl(String tableName) {
        return "create  TABLE " + tableName + "(" + "row_sequence NUMBER(19,0), " + "captured_scn NUMBER(19,0), " + "table_name VARCHAR2(30 CHAR), " + "seg_owner VARCHAR2(30 CHAR), " + "operation_code NUMBER(19,0), " + "change_time TIMESTAMP(6), " +
                ...
                // "ssn NUMBER(19,0)," +
                "csf NUMBER(19,0), " + "redo_sql VARCHAR2(4000 CHAR)" +
    ...
    public static String descTableStatement(String schema, String tableName) {
        return String.format("select " + "owner, table_name, column_name, data_type as column_type " + "from all_tab_columns " + "where  owner = upper('%s') and  table_name = upper('%s')", schema, tableName);
    }
}
